...
public class WaypointsGuiRendererMixin {
    ...
    private int modifyFontWidthForCustomIcons(int original, GuiGraphics guiGraphics, MinimapRendererHelper rendererHelper, Waypoint w) {
    ...
    @WrapOperation(method = "drawIconOnGUI", at = @At(value = "INVOKE", target = "Lxaero/common/minimap/render/MinimapRendererHelper;addColoredRectToExistingBuffer(Lorg/joml/Matrix4f;Lcom/mojang/blaze3d/vertex/VertexConsumer;FFIIFFFF)V", remap = true))
    private void drawCustomIcon(MinimapRendererHelper instance, Matrix4f matrix, VertexConsumer vertexBuffer, float x, float y, int w, int h, float r, float g, float b, float a, Operation<Void> original, GuiGraphics guiGraphics, MinimapRendererHelper rendererHelper, Waypoint waypoint) {
        if (waypoint.getSymbol().startsWith("minimapsync_")) {
            XaerosMapCompat.drawCustomIcon(guiGraphics.pose(), waypoint.getSymbol(), x, y, w, h, r, g, b, a);
        } else {
